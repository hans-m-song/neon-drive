...
import utils.lab_utils as lu
from utils.lab_utils import Mat3, Mat4, make_translation, normalize
...
def buildShader(
    vertexShaderSource, fragmentShaderSource, attribLocs, fragDataLocs={}
):
    shader = glCreateProgram()

    if compileAndAttachShader(
        shader, GL_VERTEX_SHADER, vertexShaderSource
    ) and compileAndAttachShader(
...
def drawVertexDataAsTriangles_part3(
    triangleVerts, transform, patternThreshold
):
    def setUniforms(shader):
        glUniform1f(
            glGetUniformLocation(shader, "patternThreshold"), patternThreshold
        )
...
def drawVertexDataAsTrianglesWithShaders(
    triangleVerts,
    tfm,
    ...
    if (
        ...
        or g_fragmentShaderSourceCode != fragmentShaderSourceCode
        or not g_userShader
    ):
        if g_userShader != 0:
            glDeleteProgram(g_userShader)
        g_vertexShaderSourceCode = vertexShaderSourceCode
        g_userShader = buildBasicShader(
    ...
    transformUniformIndex = glGetUniformLocation(
        g_userShader, "transformationMatrix"
    )
...
def drawMagnifiedRegion(lowerLeft, size, magFactor):
    ...
    pixel_data = glReadPixels(
        lowerLeft[0], lowerLeft[1], size[0], size[1], GL_BGR, GL_UNSIGNED_BYTE
    )
...
def drawObjModel(viewToClipTfm, worldToViewTfm, modelToWorldTfm, model):
    ...
    transforms = {
        "modelToClipTransform": viewToClipTfm
        * worldToViewTfm
        * modelToWorldTfm,
...
def drawCoordinateSystem(viewToClipTfm, worldToViewTfm):
    ...
    glUniform3fv(
        glGetUniformLocation(
            g_coordinateSystemModel.defaultShader, "viewSpaceLightDirection"
        ),
        1,
        viewSpaceLightDirection,
    )
...
def initGlFwAndResources(title, startWidth, startHeight, initResources):
    ...
    (g_vertexDataBuffer, g_vertexArrayObject) = createVertexArrayObject(
...
def setupFbo(
    ...
    glRenderbufferStorageMultisample(
        GL_RENDERBUFFER, numSamples, GL_RGB8, fboWidth, fboHeight
    )
    glBindRenderbuffer(GL_RENDERBUFFER, depthRenderBuffer)
    glRenderbufferStorageMultisample(
        GL_RENDERBUFFER, numSamples, GL_DEPTH_COMPONENT32, fboWidth, fboHeight
    )
